...
```{r}
required_packages <- c("dplyr", "purrr", "rsample", "glmnet")
```
...
```{r}
make_clinical_scores <- function(x, temp, pulse, mixture, penalty, by) {
...
#  model <- glm(BloodCSResult ~ ., binomial, x)       ### HERE WE WANT AUTO-TUNED LASSO
  model <- glmnet(Xs, Y, binomial, alpha = mixture, lambda = penalty)                  # lambda, alpha
...
# 1. recoding the continuous variables:
  x <- x |> 
    mutate(across(OralTemperature, ~ .x > temp),                        # temp
           across(Pulse, ~ .x > pulse))                                 # pulse
  xfactors <- model.matrix(BloodCSResult ~ ., x)[, -1]

# 2. fitting a logistic model:
  model <- glmnet(x, as.factor(x$BloodCSResult), binomial, alpha = mixture, lambda = penalty)
  model <- glmnet(Xs, Y, binomial, alpha = mixture, lambda = penalty)   # lambda, alpha
# 3. generating the clinical scores:
  cscore <- coef(model)[-1]
  cscore |>
...
```{r}
make_clinical_scores(train_data, 38.5, 125, .5, 1, 1)
```
