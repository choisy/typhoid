...
Computing the scores of the first row, for various values of `n`:

```{r}
the_scores <- map(n_vals, coef2scores, x = unlist(row1[["coeffs"]]))
```

For the first score (i.e. first value of `n`) of the first row:
...
compute_j <- function(test, temp, pulse, score, threshold) {
  tibble(obsv = log2fac(test$BloodCSResult),
         pred = log2fac(make_points(
           recoding(test, temp, pulse)[, -1], score) > threshold)) |> 
    table() |>
    j_index()                                           # could be replaced by accuracy
}
```
...
```{r}
#map_dbl(the_thresholds, compute_j, test = train_data, temp = row1$temp, pulse = row1$pulse, score = score1)
```
...
# j_from_threshold(train_data, row1$temp, row1$pulse, score1, the_thresholds)
```
...
matrix_from_threshold(train_data, row1$temp, row1$pulse, score1, the_thresholds)
```

```{r}
a <- train_data |> 
  bootstraps2() |> 
  map_dfc(j_from_threshold, temp = row1$temp, pulse = row1$pulse, score = score1,
...
This gives you the best threshold for a given value of `n` (that determines `score1`)

```{r}
train_data |> 
  bootstraps2() |> 
  map(matrix_from_threshold, temp = row1$temp, pulse = row1$pulse, score = score1,
      thresholds = the_thresholds) |> 
  map(map, j_index) |> 
  unlist() |> 
  matrix(length(the_thresholds)) |> 
  rowMeans() |> 
  which.max() |> 
  extract2(the_thresholds)
```
...
find_best_threshold(train_data, row1$temp, row1$pulse, score1, the_thresholds)
```
...
marc <- map(the_scores, ~ find_best_threshold(train_data, row1$temp, row1$pulse,
                                              .x, make_thresholds(.x)))
```

```{r}
aa <- map(marc, ~ .x[which.max(.x)])
which_n <- which.max(aa)
n_vals[which_n]
names(aa[[which_n]])
```

Meaning that the optimal number of categories to split the coefficients values and the
optimal threshold are respectively `n_vals[which_n]` and `as.integer(names(aa[[which_n]]))`
...
```{r}
bootstrapJ <- function(test, temp, pulse, score, threshold, ...) {
  test |> 
    bootstraps2(...) |>
    map_dbl(compute_j, temp, pulse, score, threshold) |>
    mean()
}

# bootstrapJ(train_data, row1$temp, row1$pulse, score1, the_thresholds[[1]])
```

```{r}
map_dbl(the_thresholds, boot)
```
